...
contains
    ...
    function parabola_integrate(a, b, n, cfs) result(approx)
        implicit none
        real*8, intent(in) :: a, b
        integer, intent(in) :: n
        ...
        p = partition(a, b, n)
        do i = 2, SIZE(p), 2
            insum = insum + 4*evaluate(cfs, p(i)) + 2*evaluate(cfs, p(i+1))
        end do

        approx = (delta(a, b, n)/3)* insum 
    end function parabola_integrate
...
program program40
    ...
    print *, parabola_integrate(a, b, n, cfs)
