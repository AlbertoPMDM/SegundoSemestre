...
class utils:
    ...
    def parabola_integrate(a:float, b:float, n:int, cfs:list) -> float:
        insum = utils.evaluate(cfs, a) + utils.evaluate(cfs, b)
        p = utils.partition(a, b, n)
        for x in range(1,len(p)-1, 2):
            insum += 4*utils.evaluate(cfs, p[x]) + 2*utils.evaluate(cfs, p[x+1])

        return (utils.delta(a, b, n)/3)* insum  
...
if __name__ == '__main__':
    ...
    n = int(input('introduce el numero de particiones a utilizar (debe de ser par)\n'))

    print(f'''la integral cerca de x = {r[0]} a x = {r[1]} es aproximadamente {
        utils.parabola_integrate(r[0], r[1], n, a)
        }''')